...
- The top bar now includes git-aware file controls:
  - `diff` toggles a read-only side-by-side comparison view (`HEAD` on the left, current in-memory editor content on the right).
  - Diff mode highlights removed/changed left-side lines in red and added/changed right-side lines in green.
  - Diff mode compares against the currently committed Git `HEAD` version regardless of staged/unstaged local state.
  - The terminal-pane toggle button is hidden while diff mode is active so pane-collapse state cannot be changed from review mode.
...
- `src/renderer/GitDiffPane.tsx`: read-only side-by-side diff renderer for comparing the current editor content against the file's committed `HEAD` snapshot.
...
- `src/renderer/git-diff.ts`: pure side-by-side line-diff model builder used by diff mode (LCS alignment with a large-file fallback path).
