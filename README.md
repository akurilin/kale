...
**How it works:** Setting `KALE_CDP_PORT` causes the main process (`src/main.ts`) to enable Chromium's remote debugging port. Playwright then connects over CDP and gets a standard `Page` object for the app window.
...
- The CDP port defaults to 9222 and can be overridden with the `KALE_CDP_PORT` env var.
