...
Comments are in the file (so agents see them) but collected in one place rather than scattered inline. Connection between instruction and text is indirect — the agent must search for the quoted anchor text.
